...
class DropDownList extends StatefulWidget {
  ...
  const DropDownList(
      {Key key, this.onValueSelected, this.hintText, this.givenEnumClass})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Center(
      child: DropdownButton<String>(
        items: (widget.givenEnumClass == "PetType")
            ? PetType.values
                .where((type) => (type != PetType.NotDefined))
                .map((PetType classType) {
                return DropdownMenuItem<String>(
                    value: EnumToString.parse(classType),
                    child: Text(EnumToString.parse(classType)));
              }).toList()
            : (widget.givenEnumClass == "ServiceCategory")
                ? ServiceCategory.values
                    .where(
                        (category) => (category != ServiceCategory.NotDefined))
