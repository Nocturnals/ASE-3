...
class FirebaseServicesService extends ServicesService {
  ...
  Future<String> deleteService(String serviceId) async {
    await _firestore.collection("services").document(serviceId).delete();
    return serviceId;
  }
  ...
  @override
  Future<List<Service>> getServicesForSearch(
      String petType, String category) async {
    if (category == null)
  ...
  Future<List<Service>> getServicesForPetTypeAndCategory(
      String petType, String category) async {
    final querySnapshot = await _firestore
        .collection("services")
        .where("pet_type", isEqualTo: petType)
